...
def preprocess_data(data):
    ...
    # Reshape images
    processed_images = images.reshape((images.shape[0], 27, 19, 1)).astype('float32')

    return processed_images, labels

# Load data
with open('../Data/output_4x100x10.json', 'r') as file:
    data = json.load(file)

# Preprocess data
processed_images, labels = preprocess_data(data)
...
datagen = tf.keras.preprocessing.image.ImageDataGenerator(
    ...
    width_shift_range=0.3,  # Shift images horizontally by up to 10%
    height_shift_range=0.3,  # Shift images vertically by up to 10%
...
class_weights = {0: 1, 1: 1, 2: 1, 3: 0.7}
